...
if [[ $CMDLINE != "${CMDLINE/SM-N900P}" ]]; #hltespr
    ...
    then
       sed -i 's|ro.build.fingerprint=xxxxxxxxxx|ro.build.fingerprint=samsung/hltexx/hlte:4.3/JSS15J/N9005XXUBMH1:user/release-keys|g' /tmp/build.prop
       sed -i 's|ro.build.description=xxxxxxxxxx|ro.build.description=hltexx-user 4.3 JSS15J N9005XXUBMH1 release-keys|g' /tmp/build.prop
